...
def main():
    ...
    finance_markup = st.number_input("Finance Markup", value=0.0, format="%.1f", args={})
    lease_markup = st.number_input("Lease Markup", value=0.0, format="%.5f", args={})
    ...
    for i in range(num_rows):
        ...
        st.session_state.tiers[i]["minValue"] = st.number_input(f"Minimum Credit Score", value=st.session_state.tiers[i]["minValue"],
                                               key=f"min_score_{i}", min_value=300, max_value=850, step=1, args={})
        ...
        if custom_markup:
            finance_value = st.number_input(f"Custom Finance Value", key=f"finance_value_{i}", format="%.1f", args={})
            lease_value = st.number_input(f"Custom Lease Value", key=f"lease_value_{i}", format="%.5f", args={})
        else:
        ...
        if customize_used:
            used_finance_value = st.number_input(f"Used Finance Value", key=f"used_finance_value_{i}", format="%.1f", args={})
            used_lease_value = st.number_input(f"Used Lease Value", key=f"used_lease_value_{i}", format="%.5f", args={})
        else:
